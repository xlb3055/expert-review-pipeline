...
"""
火山引擎通用查多条节点：按运行时参数从任意飞书多维表查询多条记录字段。

示例:
python3 scripts/feishu_query_node.py \
  --app-token app_token \
  --table-id table_id \
  --fields-json '{"expert_name":"提交人","status":"审核状态"}' \
  --search-body-json '{"filter":{"conjunction":"and","conditions":[{"field_name":"审核状态","operator":"is","value":["待审"]}]}}' \
  --max-records 200 \
...
from core.feishu_nodes import load_json_object, query_records_to_data
...
def main() -> int:
    parser = argparse.ArgumentParser(description="飞书通用查多条节点")
    ...
    parser.add_argument("--search-body-json", default="", help="查询条件 JSON 字符串，会原样传给飞书 search 接口")
    parser.add_argument("--search-body-file", default="", help="查询条件 JSON 文件路径")
    parser.add_argument("--page-size", type=int, default=100, help="单页条数，默认 100")
    parser.add_argument("--max-records", type=int, default=0, help="最多取多少条；0 表示取完")
    ...
    if args.output_file:
        ...
        print(f"已写出 {payload['count']} 条记录到: {args.output_file}")
    else:
