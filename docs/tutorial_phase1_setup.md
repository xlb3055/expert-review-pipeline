...
将飞书多维表格中的「开始评审」按钮与火山引擎流水线打通。专家点击按钮后，飞书自动发送 HTTP 请求携带当前行的 `record_id`，触发火山流水线自动执行粗筛 → AI 评审 → 回填。
...
- Phase 5 全部完成（火山引擎流水线手动运行通过）
- 已记下火山引擎的 Webhook URL
- 飞书表格中已有「开始评审」按钮字段
...
1. 触发条件选择：**「当点击按钮时」**
2. 选择数据表和按钮字段 **「开始评审」**
3. 如果按钮字段需要关联自动化，先保存工作流，再到按钮字段设置中关联

### Step 3：配置 HTTP 请求动作

添加动作节点，选择 **「发送 HTTP 请求」**，配置如下：
...
填入火山引擎 Webhook URL（Phase 5 Step 4 记下的）：

```
https://cp.volces.com/v2/webhook/你的WebhookID?workspaceId=你的workspaceId&pipelineId=你的pipelineId
```
...
选择 **Raw 格式（JSON）**，输入内容需要**手动拼接 JSON + 引用变量**：

1. 先输入：`{"record_id":"`
2. 点击 **⊕** 按钮，选择 **「第 1 步触发时」** → **「record_id」** 字段
3. 再输入：`"}`

最终请求体看起来是：

```
{"record_id":"[第1步触发时 → record_id]"}
```

> **关键**：引号必须紧贴变量引用标签，中间不能有空格。如果 JSON 格式不对，火山引擎会返回 `parse webhook json err` 错误。
...
如果左侧显示「请在所需的按钮字段或按钮组件中关联此自动化流程」：

1. 先点右下角 **「仅保存」**
2. 关闭自动化编辑器
3. 回到表格，点击「开始评审」列头
4. 在按钮字段配置中，选择「关联自动化」→ 选择刚创建的工作流

如果按钮字段没有「关联自动化」选项，可以在自动化的触发器配置中直接选择按钮字段。

### Step 5：保存并启用

点击 **「保存并启用」**（或「仅保存」后手动启用）。

### Step 6：端到端测试

1. 在飞书表格中找到一行有完整测试数据的记录
2. 点击该行的 **「开始评审」** 按钮
3. 观察以下变化：
   - 飞书自动化执行记录中显示 HTTP 请求已发送
   - 火山引擎流水线被触发并开始运行
   - 等待 3-5 分钟后，表格中的审核结果字段自动更新
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| `parse webhook json err` (500) | 请求体不是合法 JSON | 确保变量引用两侧紧贴引号，不能有空格 |
| `HTTP 请求出参返回格式不正确` | 火山引擎 Webhook 返回空响应体 | **可忽略**，不影响实际触发，流水线正常执行 |
| `RECORD_ID 未设置` | Webhook 参数未映射到环境变量 | 火山引擎触发器配置中，运行时变量填 `$.record_id` |
| 按钮点击无反应 | 自动化未关联按钮字段 | 在按钮字段配置中关联自动化工作流 |
| 按钮点击后表格无变化 | 流水线未执行或回填失败 | 去火山引擎控制台查看运行日志 |

### 关于飞书响应提示

飞书自动化执行后可能会显示「HTTP 请求出参返回格式不正确」——这是因为火山引擎 Webhook 成功后返回 `HTTP 200` 空响应体，飞书期望收到 JSON 格式的响应。**这个提示不影响任何功能**，流水线已正常触发执行。

---

## 全流程总结

至此，专家考核产物自动审核流水线已全部搭建完成。回顾 6 个阶段：

| 阶段 | 内容 | 产出 |
|------|------|------|
| Phase 1 | 代码编写 + 飞书表格 + API 配通 | 代码推送 GitHub，飞书 API 读写正常 |
| Phase 2 | 粗筛测试 | 6 项检查正常运行，结果回填飞书 |
| Phase 3 | AI 评审测试 | Daytona 沙箱 + Claude 评分正常 |
| Phase 4 | 回填 + 端到端串联 | 本地完整流程跑通 |
| Phase 5 | 火山引擎流水线上线 | 流水线部署，Webhook 触发正常 |
| Phase 6 | 飞书按钮对接 | 点击按钮自动触发全流程 |

### 完整执行流程

```
专家在飞书表格填写：任务描述 + 上传 Trace + 提供最终产物
    ↓
点击「开始评审」按钮
    ↓
飞书自动化 → POST record_id → 火山引擎 Webhook
    ↓
火山引擎流水线启动（自动拉取 GitHub 代码）
    ↓
阶段1：粗筛（~5秒）
  ├─ 6 项硬性检查
  ├─ 不通过 → 回填「拒绝」→ 结束
